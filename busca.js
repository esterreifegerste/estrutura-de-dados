// BUSCA SEQUENCIAL

var valores = [5, 8, 10, 22, 45, 38];

function busca(num) {
    for(i=0; i < 6; i++) {
        if (num == valores[i]) {
            return i;
        }
    }
    return -1;
}

// usando a nossa ferramenta de busca

console.log(busca(10)); //tem que retornar o valor 2 (a posicao dele)
console.log(busca(50)); //tem que retornar -1(ou seja, nao foi encontrado)



// BUSCA BINARIA (pre-requisito: os valores tem que estar ordenados)

var valores = [5, 8, 10, 22, 38, 45, 60, 90, 100, 120];

function busca(num) {
    for(i=0; i < 6; i++) {
        if (num == valores[i]) {
            return i;
        }
    }
    return -1;
}

function buscaBin(num) {
    let inicio, meio, fim;
    let passos = 0;
    inicio = 0;
    fim = 9;
    while (inicio <= fim) {
        meio = parseInt(inicio + fim) / 2;
        passos = passos + 1;
        if (num == valores[meio]) {
            console.log("Achei em "+passos+" passos");
            return meio;
        }
        else {
            if (num > valores[meio]) {
                inicio = meio + 1;
            }
            else {
                fim = meio - 1;
            }
        }

    }
    console.log("nao achei em "+passos+"passos");
    return -1;
}

// usando a nossa ferramenta de busca
console.log(buscaBin(10));
console.log(buscaBin(50));
console.log(buscaBin(60));
